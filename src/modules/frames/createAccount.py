...
class CreateAccount(tk.Frame):
    ...
    def __init__(self, parent,controller):
       ...
       # self.image = tkinter.PhotoImage(file="./src/images/stellarbot.png")
        ...
        self.screen_info_canvas = tk.Canvas(self, width=1530, height=600, background='black')
        ...
        self.go_back_account_button = tk.Button(self, text="Go Back",font=("Helvetica",15),command=lambda: self.controller.show_pages("Login"))   
        self.go_back_account_button.place(x=420,y=610)


        self.create_account_button = tk.Button(self, text="Create New Account",command=lambda:self.create_account_now(),font=("Helvetica",15))
        self.create_account_button.place(x=520,y=610)
        self.config(background='#1e2a38')

      
        self.config(bg='#1e2a38', width=1530,height = 780)
        self.welcome_label.config(padx=0,pady=0,bg='green',fg='white',width=1530,height=800,anchor= 'n')
    ...
    def create_accounts(self):
        try:
            ...
            return  {'account_id' : e, 'secret_key' : ''}
