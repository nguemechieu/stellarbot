...
def generate_cmf_signal(signal_setting, ohclv_data):
    ...
    data['N'] = (data['high'].shift(signal_setting['period']) + data['low'].shift(signal_setting['period']) + data[
        'close'].shift(signal_setting['period'])) / 3
    data['CMF'] = (data['N'] - data['M']) / (data['M'].rolling(window=signal_setting['period']).mean()) * 100
    data['CMF_Signal'] = np.where(data['CMF'] < -signal_setting['overbought_threshold'], 1,
                                  np.where(data['CMF'] > signal_setting['oversold_threshold'], -1, 0))
...
def generate_willr_signal(signal_setting, ohclv_data):
    ...
    data['WillR_Signal'] = np.where(data['WillR'] < -signal_setting['overbought_threshold'], 1,
                                    np.where(data['WillR'] > signal_setting['oversold_threshold'], -1, 0))
    data['Position'] = data['WillR_Signal'].diff()
    return \
    data[['close', 'TR', 'ATR', '+DM', '-DM', '+DI', '-DI', 'WillR', 'WillR_Signal', 'Position']].tail(1).to_dict(
        orient='records')[0]
...
def generate_stoch_signal(signal_setting, ohclv_data):
    ...
    data = ohclv_data.copy()
    data['K'] = ((data['high'] - data['close']) / (
                data['high'] - data['low'].rolling(window=signal_setting['period']).min())) * 100
    data['D'] = data['K'].rolling(window=signal_setting['period']).mean()
    data['Stochastic_Signal'] = np.where(data['D'] < signal_setting['oversold_threshold'], 1,
                                         np.where(data['D'] > signal_setting['overbought_threshold'], -1, 0))
...
def generate_custom_indicator_signal(signal_setting, ohclv_data_):
    #  Custom indicator strategy logic
    data = ohclv_data_.copy()
    ...
    data['Custom_Indicator_Signal'] = np.where(data['Custom_Indicator'] > 0, 1,
                                               np.where(data['Custom_Indicator'] < 0, -1, 0))
    data['Position'] = data['Custom_Indicator_Signal'].diff()
    return data[['close', 'Custom_Indicator', 'Custom_Indicator_Signal', 'Position']].tail(1).to_dict(orient='records')[
        0]
...
def generate_atr_signal(signal_setting, ohclv_data):
    ...
    data['ATR'] = data['TR'].rolling(window=signal_setting['period']).mean()
    data['ATR_Signal'] = np.where(data['close'] > data['close'].shift() + data['ATR'], 1,
                                  np.where(data['close'] < data['close'].shift() - data['ATR'], -1, 0))
    data['Position'] = data['ATR_Signal'].diff()
    return \
    data[['close', 'H-L', 'H-PC', 'L-PC', 'TR', 'ATR', 'ATR_Signal', 'Position']].tail(1).to_dict(orient='records')[0]
...
def generate_dmi_signal(signal_setting, ohclv_data):
    ...
    data['+DM'] = np.maximum(data['high'] - data['high'].shift(1), 0)
    data['-DM'] = np.maximum(data['low'].shift(1) - data['low'], 0)
    data['+DI'] = data['+DM'].rolling(window=signal_setting['period']).sum() / data['+DM'].rolling(
        window=signal_setting['period']).sum().rolling(window=signal_setting['period']).mean()
    data['-DI'] = data['-DM'].rolling(window=signal_setting['period']).sum() / data['-DM'].rolling(
        window=signal_setting['period']).sum().rolling(window=signal_setting['period']).mean()
    data['DX'] = 100 * np.abs(data['+DI'] - data['-DI']) / (data['+DI'] + data['-DI'])
    data['ADX'] = data['DX'].rolling(window=signal_setting['period']).mean()
    data['ADX_Signal'] = np.where(data['ADX'] < signal_setting['oversold_threshold'], 1,
                                  np.where(data['ADX'] > signal_setting['overbought_threshold'], -1, 0))
...
def generate_ichimoku_signal(signal_setting, ohclv_data):
    ...
    data = ohclv_data.copy()
    data['Tenkan_Span'] = (data['high'].rolling(window=signal_setting['tenkan_period']).max() + data['low'].rolling(
        window=signal_setting['tenkan_period']).min()) / 2
    ...
    data['Senkou_B'] = ((data['high'].rolling(window=signal_setting['senkou_b_period']).max() + data['low'].rolling(
        window=signal_setting['senkou_b_period']).min()) / 2).shift(signal_setting['senkou_b_period'])
    ...
    data['Ichimoku_Signal'] = np.where(data['Conversion_Line'] > data['Signal_Line'], 1,
                                       np.where(data['Conversion_Line'] < data['Signal_Line'], -1, 0))
    data['Position'] = data['Ichimoku_Signal'].diff()
    return data[
        ['close', 'Tenkan_Span', 'Kijun_Span', 'Senkou_A', 'Senkou_B', 'Chikou_Span', 'Conversion_Line', 'Base_Line',
         'Signal_Line', 'Ichimoku_Signal', 'Position']].tail(1).to_dict(orient='records')[0]


def generate_aroon_signal(signal_setting, ohclv_data):
    #  Aroon strategy logic
    data = ohclv_data.copy()
    data['Aroon_Up'] = 100 - (100 / (data['high'].rolling(window=signal_setting['period']).max() - data['low'].rolling(
        window=signal_setting['period']).min()).rolling(window=signal_setting['period']).mean())
    data['Aroon_Down'] = 100 - (100 / (
                data['high'].rolling(window=signal_setting['period']).max() - data['low'].rolling(
            window=signal_setting['period']).min()).rolling(window=signal_setting['period']).mean())
...
def generate_uo_signal(signal_setting, ohclv_data):
    ...
    data['UO_Signal'] = np.where(data['UO'] > signal_setting['overbought_threshold'], 1,
                                 np.where(data['UO'] < signal_setting['oversold_threshold'], -1, 0))
...
def generate_kdj_signal(signal_setting, ohclv_data):
    #  KDJ strategy logic
    data = ohclv_data.copy()
    data['RSV'] = (data['close'] - data['low'].rolling(window=signal_setting['period']).min()) / (
                data['high'].rolling(window=signal_setting['period']).max() - data['low'].rolling(
            window=signal_setting['period']).min()) * 100
    ...
    data['KDJ_Signal'] = np.where(data['J'] < signal_setting['overbought_threshold'], 1,
                                  np.where(data['J'] > signal_setting['oversold_threshold'], -1, 0))
...
def generate_ppo_signal(signal_setting, ohclv_data):
    ...
    data['PPO_Signal'] = np.where(data['PPO'] < -signal_setting['overbought_threshold'], 1,
                                  np.where(data['PPO'] > signal_setting['oversold_threshold'], -1, 0))
...
def generate_trix_signal(signal_setting, ohclv_data_):
    #  TRIX strategy logic
    data = ohclv_data_.copy()
    data['TR'] = np.maximum(np.maximum(data['high'] - data['low'], abs(data['high'] - data['close'].shift(1))),
                            abs(data['low'] - data['close'].shift(1)))
    ...
    data['Trix_Signal'] = np.where(data['trix'] < -signal_setting['overbought_threshold'], 1,
                                   np.where(data['trix'] > signal_setting['oversold_threshold'], -1, 0))
    data['Position'] = data['Trix_Signal'].diff()
    return data[['close', 'TR', 'TR14', 'trix', 'Trix_Signal', 'Position']].tail(1).to_dict(orient='records')[0]


def generate_bbands_signal(signal_setting, ohclv_data__):

    data = ohclv_data__.copy()
    data['Middle_Band'] = data['close'].rolling(window=signal_setting['bbands_period']).mean()
    data['Upper_Band'] = data['Middle_Band'] + (
                signal_setting['bbands_std_dev'] * data['close'].rolling(window=signal_setting['bbands_period']).std())
    data['Lower_Band'] = data['Middle_Band'] - (
                signal_setting['bbands_std_dev'] * data['close'].rolling(window=signal_setting['bbands_period']).std())
    data['Bollinger_Band_Signal'] = np.where(data['close'] < data['Lower_Band'], 1,
                                             np.where(data['close'] > data['Upper_Band'], -1, 0))
    data['Position'] = data['Bollinger_Band_Signal'].diff()
    return \
    data[['close', 'Middle_Band', 'Upper_Band', 'Lower_Band', 'Bollinger_Band_Signal', 'Position']].tail(1).to_dict(
        orient='records')[0]
...
def generate_adx_signal(signal_setting, ohclv_data):
    ...
    data['ADX_Signal'] = np.where(data['adx'] < signal_setting['overbought_threshold'], 1,
                                  np.where(data['adx'] > signal_setting['oversold_threshold'], -1, 0))
    data['Position'] = data['ADX_Signal'].diff()
    return data[['close', 'true_range', 'plus_dm', 'minus_dm', 'dx', 'adx', 'ADX_Signal', 'Position']].tail(1).to_dict(
        orient='records')[0]
...
def generate_rsi_signal(signal_setting, ohclv_data):
    ...
    data['RSI_Signal'] = np.where(data['rsi'] < signal_setting['overbought_threshold'], 1,
                                  np.where(data['rsi'] > signal_setting['oversold_threshold'], -1, 0))
    data['Position'] = data['RSI_Signal'].diff()
    return \
    data[['close', 'delta', 'gain', 'loss', 'avg_gain', 'avg_loss', 'rsi', 'RSI_Signal', 'Position']].tail(1).to_dict(
        orient='records')[0]
...
def generate_macd_signal(signal_setting, ohclv_data):
    #  MACD strategy logic
    data = ohclv_data.copy()
    data['EMA_fast'] = data['close'].ewm(span=signal_setting['fast_ema_period'],
                                         min_periods=signal_setting['fast_ema_period']).mean()
    data['EMA_slow'] = data['close'].ewm(span=signal_setting['slow_ema_period'],
                                         min_periods=signal_setting['slow_ema_period']).mean()
    data['MACD'] = data['EMA_fast'] - data['EMA_slow']
    data['Signal'] = data['MACD'].ewm(span=signal_setting['signal_ema_period'],
                                      min_periods=signal_setting['signal_ema_period']).mean()

    data['MACD_Signal'] = np.where(data['MACD'] > data['Signal'], 1, np.where(data['MACD'] < data['Signal'], -1, 0))
    data['Position'] = data['MACD_Signal'].diff()
    return data[['close', 'EMA_fast', 'EMA_slow', 'MACD', 'Signal', 'MACD_Signal', 'Position']].tail(1).to_dict(
        orient='records')[0]
...
def generate_bollinger_bands_signal(signal_settings, ohclv_data):
    ...
    data['Upper_Band'] = data['Middle_Band'] + (signal_settings['bbands_std_dev'] * data['close'].rolling(
        window=signal_settings['bbands_period']).std())
    data['Lower_Band'] = data['Middle_Band'] - (signal_settings['bbands_std_dev'] * data['close'].rolling(
        window=signal_settings['bbands_period']).std())
    data['Bollinger_Band_Signal'] = np.where(data['close'] < data['Lower_Band'], 1,
                                             np.where(data['close'] > data['Upper_Band'], -1, 0))
    data['Position'] = data['Bollinger_Band_Signal'].diff()
