...
import numpy as np
from any_json_to_data_frame import Any_json_to_data_frame
import base64
...
class TradingBot:
    ...
    def __init__(self, account_id=None, account_secret=None,controller=None):   
        ...
        self.thread = threading.Thread(target=self.on_tick, args=())
        try:
        ...
        self.db.commit()
        self.any_json_to_data_frame = Any_json_to_data_frame()
    ...
    # Used to learn the model
        ...
        self.counter_asset_code='AQUA'
        self.counter_asset_issuer ="GBNZILSTVQZ4R7IKQDGHYGY2QXL5QOFJYQMXPKWRRM5PAV7Y4M67AQUA"
        ...
        self.symbol = self.counter_asset.__str__()+'/'+self.base_asset.__str__()
        self.server_msg['message'] = self.symbol
        ...
        self.server_msg['status'] = 'CONNECTED TO STELLAR ACCOUNT'
        ...
        self.order_book_df = pd.DataFrame(self.order_book)
        self.order_book_df.to_csv('ledger_order_book.csv', index=0)
        fee_statistic =self.server.fee_stats().limit(100).order(desc=True).call().popitem()[-1]
        self.fee_statistics_df=self.any_json_to_data_frame.convert_json_to_data_frame(fee_statistic)
        self.fee_statistics_df.to_csv('ledger_fee_statistics.csv',index=True)
        stellar_offers_json = self.fetch_stellar_offers()
        self.ledger_offers_df=self.any_json_to_data_frame.convert_json_to_data_frame(stellar_offers_json)
        ...
        transaction =self.server.transactions().limit(100).order(desc=True).call().popitem()[-1]['records']
        ...
        self.claimable_df=self.any_json_to_data_frame.convert_json_to_data_frame(self.claimable)
    ...
    def get_signal(self, symbol: str): 
        ...
        #     if (price - fibo_price / price) < 0.01:  # You can adjust the threshold as needed
        #      signal = 1  # Buy signal
    ...
    def get_asset_list(self):
        ...
        self.asset_list.to_csv('ledger_asset_list.csv', index=False) 
        ...
        while True:
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.server_msg['message'] = 'Connection '
                ...
                tkinter.Message(self, text=self.server_msg['message']).pack()
            
                return False
            finally:
                time.sleep(0.5) 
    def convert_account_to_dataframe(self,account_data):
     if account_data is not None:
        # Extract relevant information from the JSON data into a dictionary
        account_info = {
        ...
        df = self.any_json_to_data_frame.convert_json_to_data_frame(account_info)
        df.to_csv('ledger_account_info.csv', index=False)
     else:
        return None
    ...
    # Create the first Asset instance (selling asset)
             ...
             sell_amount = "16.0"  # Amount of selling asset
             sell_price =  "0.0056400" # Price in terms of selling asset\stellar_sdk.price.Price, str, decimal.Decimal
            ...
            #Create the sell operation
             ...
             #Get SIGNAL
             while True:
               self.signal=self.get_signal(self.symbol)
    
            
               try:
                 self.server_msg['message'] = self.symbol + "signal: " + str(self.signal) + "\n"+ '\ntransaction_hash:'+ self.operation2_df['transaction_hash'].__str__() + '\n'
                
                 self.signal=-1
                 if self.signal == 1:
                  # Build buying the transaction

                  self.server_msg['status'] = 'BUYING'

                  self.trade_operation(selling_asset=self.counter_asset,buying_asset=self.base_asset,operation_type='buy',amount=30,price=7.3)

                 elif self.signal == -1:
              # Build selling the transaction
                   self.server_msg['status'] = 'SELLING'
                   self.trade_operation(selling_asset=self.counter_asset,buying_asset=self.base_asset,operation_type='sell',amount=30,price=7.3)
                 
               except Exception as e:
                  self.server_msg['message'] = 'Transaction:'+str(e)
                  self.server_msg['status'] = 'Transaction failed'
               time.sleep(1)
    ...
    def login(self, user_id:str='', secret_key:str=''):
        ...
        if self.connected:
            ...
            self.convert_account_to_dataframe(self.account_info)
    ...
    def send_money( self, amount:str="100", asset:str=stellar_sdk.Asset.native(), receiver_address:str=None):
           ...
           try:
            ...
            self.server_msg['message'] = 'Transaction hash:'+ response["hash"].__str__()
             ...
             print(f"An error occurred: {e}")
             tkinter.Message(master=self.controller, text=e, width = 50)
    ...
    def trade_operation ( self,selling_asset, buying_asset, amount, price, operation_type):
     response=None
    # Stellar testnet server
     if operation_type == "sell":
  
    # Create the manage offer operation (sell/buy)
        manage_offer_operation = stellar_sdk.TransactionBuilder(
        source_account=self.account,
    ...
    #Convert the manage offer operation to byte array
        
        manage_offer_operation_xdr = manage_offer_operation.to_xdr()
        print(f"manage_offer_operation_xdr: {manage_offer_operation_xdr}")  
        manage_offer_operation_xdr = manage_offer_operation.to_xdr_object()

    # Sign the transaction
        manage_offer_operation.sign( self.keypair.secret)
        response = requests.post(url="https://horizon.stellar.org/transactions", data={manage_offer_operation_xdr}, timeout=5000)
        ...
        selling_code=selling_asset,
        buying_code=buying_asset,
        amount=str(amount),
        price=str(price)).set_timeout(5000).build()
        manage_offer_operation.sign(self.keypair.secret)
        response =  requests.post("https://horizon.stellar.org/transactions", data=manage_offer_operation, timeout=5000)
       ...
       self.server_msg['message'] = 'Unsupported operation type'
       tkinter.Message(master=self.controller, text=self.server_msg['message'], width = 50)
    # Submit the transaction
       ...
       if response.status_code == 200:
          ...
          self.server_msg['status'] = 'Transaction failed'
          self.server_msg['message'] = response.content
    ...
    def send_transaction(self, receiver_public_key, asset_code, amount):
    # Stellar testnet server
    ...
    # Create the payment operation
      payment_operation = stellar_sdk.TransactionBuilder(
        source_account=self.account,
        network_passphrase=stellar_sdk.Network.PUBLIC_NETWORK_PASSPHRASE,
        base_fee=100,
    ).append_payment_op(
        ...
        amount=amount,
        asset_code=asset,
        asset_issuer=self.keypair.public_key ).set_timeout(30).build()

    # Sign the transaction
      payment_operation.sign(    self.keypair.secret)

    # Submit the transaction
      response =  self.server.submit_transaction(payment_operation)
